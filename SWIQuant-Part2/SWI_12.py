...
class Swi_12(object):
    ...
    def read_step(self, x, window, corr_th, f_th, length=60, mode='One', template_id=0):
        
        template = self.get_template(length, mode, template_id)
        ...
        print('Mode {} cross-correlation > {} events: {}'.format(mode, corr_th, len(ind)))

        f_x, ind = self.out_feature(f_x=f_x, f_t=f_t, th=f_th, ind=ind)
        print('Mode {} features > {} events: {}'.format(mode, f_th, len(ind)))
    ...
    def multi_read(self, window, corr_th, f_th, n):

        print('{} template with self.template.shape = {}'.format(n, self.template.shape))
        Ids = []
        for i in range(n):
            _, id, _ = self.read_step(self.smoothed_data, window, corr_th, f_th, mode='Two', template_id=i)
            Ids.append(id)
    ...
    def cluster_template(self, align_windows):
        
        cc = Cluster(data=align_windows)
        n, discard_id = cc.find_optimal_n()
        y_pred, centers = cc.do_cluster(n)

        not_discard_id = [i for i in range(n) if i not in discard_id]
        self.template = centers[not_discard_id]

        return len(not_discard_id)

    def merge_output(self, Ids):

        record = np.zeros(len(self.smoothed_data))
        for id in Ids:
            record[id] = 1
        output_id = np.where(record == 1)[0]
        return output_id
    ...
    def get_template(self, length, mode='One', template_id=0):
    ...
    def get_data_feature(self, x_smooth, ind):
        ...
        for id, dd_x, iind in zip(max_x_ids, dd_x_smooth, ind):
            ...
            i = i+1
            if len(max_id) == 0 or max_id >= len(pp_id)-1:
                continue
            min_id = max_id + 1
    ...
    def align_spike(self, spike_windows, id):
        ...
        inds = [i for i in range(n) if i not in k]
        aligned_data = aligned_data[inds]
...
class Cluster(object):
    ...
    def __init__(self, data):
        ...
        self.data = data
        self.N, self.L = data.shape[0], data.shape[1]
    
    def do_cluster(self, n_clusters):

        cluster = KMeans(n_clusters=n_clusters, random_state=10).fit(self.data)
    ...
    def find_optimal_n(self):
        ...
        for n in range(2, 100):
            y_pred, _ = self.do_cluster(n)
            out = [i for i in range(n) if np.mean(y_pred==i)<0.05]
            if len(out) != 0:
                break
        
        return n, out

    def plot_clusters(self, n_clusters, y_pred, centers):

        fig, axes = plt.subplots(1, n_clusters, figsize=[15,4])

        for i in range(n_clusters):
            ax = axes[i]
            data = self.data[y_pred==i]
            for x in data:
                ax.plot(x, c='b', alpha=0.3)
            ax.plot(np.mean(data, axis=0), c='r')
            ax.plot(centers[i,:], c='g')
            ax.set_title('cluster {}:{:.1f}%'.format(i, np.mean(y_pred==i)*100))
...
if __name__ == '__main__':
    ...
    n = swi.cluster_template(align_windows)

    corr_th = 0.7
    f_th = 0.5
    Ids = swi.multi_read(window, corr_th, f_th, n)
    id = swi.merge_output(Ids)
